...
%             Delta_Phi(deZ-1)=((2*pi)/lambda)*sum(((NTE_Z(:,deZ)-NTM_Z(:,deZ)))*dz);
 Delta_Phi(deZ-1)=((2*pi)/lambda)*Length(deZ)*(((NTE_Z(deZ)-NTM_Z(deZ))));
               MulerMatrix=[max(G_TE(:,deZ-1)).^2+max(G_TM(:,deZ-1)).^2 max(G_TE(:,deZ-1)).^2-max(G_TM(:,deZ-1)).^2 0 0;
                ...
                0 0 (2*max(G_TE(:,deZ-1))*max(G_TM(:,deZ-1))*cos(max(Delta_Phi(:,deZ-1)))) (2*max(G_TE(:,deZ-1))*max(G_TM(:,deZ-1))*sin(max(Delta_Phi(:,deZ-1))));
                0 0 -(2*max(G_TE(:,deZ-1))*max(G_TM(:,deZ-1))*sin(max(Delta_Phi(:,deZ-1)))) (2*max(G_TE(:,deZ-1))*max(G_TM(:,deZ-1))*cos(max(Delta_Phi(:,deZ-1))))];
